 ...
 *  Encrypting the same data with the same key will result in the same cipher text. This mode should NOT be used unless necessary.
 */
 ...
 * -Use a MAC to ensure that the cipher text and IV have not been modified.
...
class BlowFish
{
    ...
    /// </summary>
    /// <param name="ct">hHex string of the cipher text</param>
    ...
    private void SetupKey7(byte[] cipherKey7)
    {
        ...
        for (int i = 0; i < 18; i++)
        {
            ulong d = (ulong)(
                (
                    (key[j % cipherKey7.Length] * 256 + key[(j + 1) % cipherKey7.Length]) * 256
                    + key[(j + 2) % cipherKey7.Length]
                ) * 256
                + key[(j + 3) % cipherKey7.Length]
            );
            bf_P[i] ^= d;
    ...
    /// </summary>
    /// <param name="text">plain/cipher text</param>
